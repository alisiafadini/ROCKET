...
for epoch in tqdm(range(num_epochs)):
    ...
    tensorboard_writer = SummaryWriter(
        log_dir="tensorboard_runs/LLG_msabias_runs/{epoch}it-lr{lr}-{b}batch-{r}subr-trueBs-nodrop-{name}".format(
            epoch=num_epochs, lr=lr_s, b=num_batch, r=sub_ratio, name=name
        )
    )
    llgloss.sfc.savePDB(
        "tensorboard_runs/LLG_msabias_runs/{epoch}it-lr{lr}-{b}batch-{r}subr-trueBs-nodrop-{name}/{epoch_it}.pdb".format(
            epoch=num_epochs,
            lr=lr_s,
            b=num_batch,
            r=sub_ratio,
...
np.save(
    "tensorboard_runs/LLG_msabias_runs/{epoch}it-lr{lr}-{b}batch-{r}subr-trueBs-nodrop-{name}/mse_losses_matrix.npy".format(
        epoch=num_epochs, lr=lr_s, b=num_batch, r=sub_ratio, name=name
    ),
    mse_losses_array,
)
...
with open(
    "tensorboard_runs/LLG_msabias_runs/{epoch}it-lr{lr}-{b}batch-{r}subr-trueBs-nodrop-{name}/sigmas_by_epoch.pkl".format(
        epoch=num_epochs, lr=lr_s, b=num_batch, r=sub_ratio, name=name
    ),
    "wb",
) as file:
...
with open(
    "tensorboard_runs/LLG_msabias_runs/{epoch}it-lr{lr}-{b}batch-{r}subr-trueBs-nodrop-{name}/rbrloss_by_epoch.pkl".format(
        epoch=num_epochs, lr=lr_s, b=num_batch, r=sub_ratio, name=name
    ),
    "wb",
) as file:
