...
lr_period = "drop500"
#scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=period, eta_min=0)
...
for epoch in tqdm(range(num_epochs)):
    ...
    # MSE loss between structure factors
    loss_real = loss_function( torch.abs(Fc), torch.abs(Fs_true) )
    loss_img  = loss_function( torch.angle(Fc), torch.angle(Fs_true))

    loss = loss_real + loss_img



    #tensorboard_writer.add_scalar("Loss_real", loss_real.item(), epoch)
    tensorboard_writer.add_scalar("LR", current_lr, epoch)
    tensorboard_writer.add_scalar("Loss_tot", loss.item(), epoch)
    ...
    tensorboard_writer = SummaryWriter(
        log_dir="tensorboard_runs/Ftrue_msabias_runs/Ftrue-{epoch}it-{b}batch-{r}subr-trueBs-drop0.6-0.0-0.0-changeLR500".format(
    ...
    llgloss.sfc.savePDB(
    "tensorboard_runs/Ftrue_msabias_runs/Ftrue-{epoch}it-{b}batch-{r}subr-trueBs-drop0.6-0.0-0.0-changeLR500/{epoch_it}.pdb".format(
...
np.save(
    "tensorboard_runs/Ftrue_msabias_runs/Ftrue-{epoch}it-{b}batch-{r}subr-trueBs-drop0.6-0.0-0.0-changeLR500/mse_losses_matrix.npy".format(
        epoch=num_epochs, lr=lr_s, b=num_batch, r=sub_ratio
    ),
    mse_losses_array,
)

with open('tensorboard_runs/Ftrue_msabias_runs/Ftrue-{epoch}it-{b}batch-{r}subr-trueBs-drop0.6-0.0-0.0-changeLR500/sigmas_by_epoch.pkl'.format(
...
llgloss.sfc.atom_pos_orth = aligned_xyz
llgloss.sfc.savePDB(
    "tensorboard_runs/Ftrue_msabias_runs/Ftrue-{epoch}it-{b}batch-{r}subr-trueBs-drop0.6-0.0-0.0-changeLR500/final.pdb".format(
        epoch=num_epochs, lr=lr_s, b=num_batch, r=sub_ratio
    )
)
