from ..refinement_config import gen_config, RUNMODE, DATAMODE
import argparse, os
from loguru import logger

def parse_args():
    parser = argparse.ArgumentParser(description='Generate ROCKET configuration files')
    parser.add_argument('--mode', type=str, choices=['phase1', 'phase2', 'both'], default='both',
                        help='Execution mode: phase1, phase2, or both (default: both)')  
    parser.add_argument('--datamode', type=str, choices=['xray', 'cryoem'], help='Data mode')
    parser.add_argument('--working-dir', type=str,
                        help='Working directory path')
    parser.add_argument('--file-id', type=str,
                        help='File ID to use')
    parser.add_argument('--pre-phase1-config', type=str,
...
def cli_runconfig():
    ...
    if args.get('mode') == RUNMODE.BOTH:
        ...
        logger.info(f"Generated Phase 1 config at: {os.path.join(args.get('working_dir'), 'ROCKET_config_phase1.yaml')}")
        logger.info(f"Generated Phase 2 config at: {os.path.join(args.get('working_dir'), 'ROCKET_config_phase2.yaml')}")
    elif args.get('mode') == RUNMODE.PHASE1:
        logger.info(f"Generated Phase 1 config at: {os.path.join(args.get('working_dir'), 'ROCKET_config_phase1.yaml')}")
    else:  # PHASE2
        logger.info(f"Generated Phase 2 config at: {os.path.join(result.working_dir, 'ROCKET_config_phase2.yaml')}")
