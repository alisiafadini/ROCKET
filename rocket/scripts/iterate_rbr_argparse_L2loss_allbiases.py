...
def main():
    ...
    if args.version == 3:
        ...
        device_processed_features["msa_feat_weights"] = torch.load("/net/cci/alisia/openfold_tests/run_openfold/test_cases/3hak/outputs/3hak_it50_v5_lr0.2+5.0_batch1_subr1.0_solvTrue_scaleTrue_Ib50_w1/mul_bias_final.pt").detach().to(device=device)
        device_processed_features["msa_feat_bias"] = torch.load("/net/cci/alisia/openfold_tests/run_openfold/test_cases/3hak/outputs/3hak_it50_v5_lr0.2+5.0_batch1_subr1.0_solvTrue_scaleTrue_Ib50_w1/add_bias_final.pt").detach().to(device=device)
        ...
        print("Incorrect version specified. Exiting.")
        exit()
    ...
    for iteration in tqdm(range(args.iterations)):
        ...
        # AF2 pass
        af2_output = af_bias(feats_copy, num_iters=1, bias=True)
        ...
        #    xyz_orth_sfc, best_pos_copy, pseudo_Bs, b_thresh=args.b_threshold
        #)
