...
def make_processed_dict_from_template(template_pdb,
    ...
    if mask_sidechains_add_cb:
        ...
        projected_cb = [i for i,b,m in zip(template_idxs, 
                                           backbone_modelled, 
                                           decoy_prot.atom_mask) if m[3] == 0 and b]
        projected_cb_set = set(projected_cb)
        gly_idx = [i for i,a in enumerate(target_seq) if a == "G"]
        assert all([k in projected_cb_set for k in gly_idx if k in template_idx_set and k in backbone_idx_set])
    ...
    if device != "cpu":
        processed_feature_dict = rk_utils.move_tensors_to_device(processed_feature_dict, device=device)
