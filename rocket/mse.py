...
class MSEloss():
    ...
    def __init__(
        ...
        assert target.sequence == moving.sequence, "target pdb has different sequence with moving pdb!"
        ...
        for i, name in enumerate(moving.cra_name):
            try:
                ...
                pass
        assert (np.array(target.cra_name)[index_target] == np.array(moving.cra_name)[index_moving]).all()
        self.index_target = np.array(index_target)
        self.index_moving = np.array(index_moving)
        self.target_pos = torch.tensor(target.atom_pos, device=device, dtype=torch.float32)
    ...
    def forward(
        ...
        mse_loss = torch.mean(torch.sum((xyz_ort[self.index_moving] - self.target_pos[self.index_target])**2, dim=-1))
        return mse_loss
